...
def _get_data_parts(parts: list[Any]) -> list[DataPartContent]:
  """SDK-version-agnostic replacement for ``message.get_data_parts``.

  Walks every ``Part`` and pulls out any DataPart payloads as plain
  dicts. Mirrors the upstream signature: list-in, list-of-dict-out.
  """
  out: list[DataPartContent] = []
  for p in parts or []:
...
class BaseServerExecutor(AgentExecutor, abc.ABC):
  ...
  async def _handle_request(
    ...
    Resolution order:
      1. Explicit ``tool_hint`` data-part (deterministic fast path used
         by the MCP gateway helpers — bypasses Gemini-Flash's
         non-deterministic tool selection so identical requests can't
         resolve to different tools across retries).
      2. Gemini ``FunctionCallResolver`` over the text prompt (legacy
         ADK path).
    ...
    """
    try:
      tool_hint = message_utils.find_data_part("tool_hint", data_parts)
      if tool_hint and any(t.__name__ == tool_hint for t in self._tools):
        tool_name = tool_hint
        logging.info("Using tool (hint): %s", tool_name)
      else:
        prompt = (text_parts[0] if text_parts else "").strip()
        tool_name = self._tool_resolver.determine_tool_to_use(prompt)
        logging.info("Using tool (resolver): %s", tool_name)
  ...
  def _parse_request(
    ...
    # `a2a.utils.message.get_data_parts` was removed in newer SDKs; pull
    # the dict payloads off DataPart roots ourselves to stay compatible.
