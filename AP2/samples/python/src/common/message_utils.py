...
def get_data_parts(parts: list[Any]) -> list[dict[str, Any]]:
    """Extract every DataPart payload from a list of A2A ``Part`` objects.

    Replacement for ``a2a.utils.message.get_data_parts`` which was
    removed in newer ``a2a-sdk`` releases. Returns a list of plain
    ``dict`` payloads, in document order.
    """
