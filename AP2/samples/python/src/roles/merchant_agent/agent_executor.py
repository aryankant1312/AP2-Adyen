...
class MerchantAgentExecutor(BaseServerExecutor):
  ...
  def __init__(self, supported_extensions: list[dict[str, Any]] = None):
    ...
    agent_tools = [
        ...
        tools.create_merchant_on_file_token,
    ]
  ...
  async def _handle_request(
    ...
    `_validate_shopping_agent` already calls ``updater.failed()`` on
    rejection — issuing a second failure here causes the A2A SDK to
    raise ``RuntimeError: Task ... is already in a terminal state``.
    """
