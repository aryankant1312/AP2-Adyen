...
async def update_cart(
  ...
  try:
    ...
    # UK pharmacy POC: GBP + 0% VAT on OTC medicines (HMRC Notice 701/57).
    tax_and_shipping_costs = [
        PaymentItem(
            label="Shipping",
            amount=PaymentCurrencyAmount(currency="GBP", value=2.00),
        ),
        PaymentItem(
            label="VAT (0% — OTC medicines)",
            amount=PaymentCurrencyAmount(currency="GBP", value=0.00),
        ),
    ]
...
async def get_merchant_on_file_payment_methods(
  ...
  """Returns payment methods the merchant has on file for the user (Mode 2).

  This is the merchant-supplied counterpart to the Credentials Provider's
  `handle_search_payment_methods`. It lets the Shopping Agent populate the
  checkout payment picker from the merchant's own customer vault (backed by
  a PSP like Stripe / Adyen), so returning customers don't have to re-pick
  their card from a separate wallet.

  The response contains aliases + last4 + brand + type only. No PSP
  references, no PAN / CVV / raw VPA are ever exported.

  Request data_parts must contain:
    - user_email: the merchant's identifier for the customer.

  Response artifact contains:
    - on_file_payment_methods: list[dict] of aliases + safe metadata.
    - source: "merchant_on_file" so the shopper knows which path to take.
  """
  del debug_mode  # unused
  del current_task  # unused
  user_email = message_utils.find_data_part("user_email", data_parts)
  if not user_email:
    await _fail_task(updater, "Missing user_email.")
    return

  methods = customer_vault.get_on_file_methods(user_email)
  logging.info(
      "merchant_on_file: returning %d method(s) for %s", len(methods), user_email
  )
  await updater.add_artifact([
      Part(
          root=DataPart(
              data={
                  ...
                  "on_file_payment_methods": methods,
              }
          )
      )
  ])
...
async def create_merchant_on_file_token(
    data_parts: list[dict[str, Any]],
    updater: TaskUpdater,
    current_task: Task | None,
    debug_mode: bool = False,
) -> None:
  """Mints a short-lived PSP charge token for a user-selected on-file method.

  Flow:
    1. Shopping agent tells us which alias the user picked ("Visa ending 1111").
    2. We look up the PSP reference the merchant stored for that alias
       (alias -> psp_ref is merchant-internal; never leaves this process).
    3. We ask the PSP to mint a charge token bound to (psp_ref, user_email).
    4. We return ONLY the token + alias upward. The token is what ends up in
       the PaymentMandate's `payment_response.details.token`.
  ...
  del current_task  # unused
  user_email = message_utils.find_data_part("user_email", data_parts)
  alias = message_utils.find_data_part("payment_method_alias", data_parts)
  if not user_email or not alias:
    await _fail_task(
        updater, "user_email and payment_method_alias are required."
    )
    return

  psp_ref = customer_vault.resolve_alias_to_psp_ref(user_email, alias)
  if not psp_ref:
    await _fail_task(
        updater,
        f"No on-file method found for alias '{alias}' on {user_email}.",
    )
    return

  charge_token = psp_vault.mint_charge_token(psp_ref, user_email)
  logging.info(
      "merchant_on_file: minted charge token for alias=%s user=%s",
      alias, user_email,
  )
  await updater.add_artifact([
      Part(
