...
from .crypto import MandateSigner
...
# Persistent ECDSA P-256 keypair used to sign PaymentMandates. Loaded lazily on
# first use so that module import does not touch the filesystem and tests can
# substitute their own signer via `set_mandate_signer`.
_mandate_signer: MandateSigner | None = None


def get_mandate_signer() -> MandateSigner:
  """Return the process-wide MandateSigner, creating it on first call."""
  global _mandate_signer
  if _mandate_signer is None:
    _mandate_signer = MandateSigner.load_or_create()
  return _mandate_signer


def set_mandate_signer(signer: MandateSigner | None) -> None:
  """Inject (or reset) the MandateSigner. Used by tests."""
  global _mandate_signer
  _mandate_signer = signer
...
def sign_mandates_on_user_device(tool_context: ToolContext) -> str:
  """Signs the transaction details with the local ECDSA P-256 keypair.

  This step represents the user's authorization on a trusted surface: the
  Shopping Agent composes a signing payload that binds the CartMandate hash,
  the PaymentMandateContents hash, a fresh nonce, and the signer's did:key
  identity, then signs it with ECDSA P-256 (SHA-256).

  The resulting compact token (base64url(header).base64url(signature)) is
  stored on the PaymentMandate's `user_authorization` field. The keypair is
  loaded or generated lazily and persisted at `~/.ap2/shopper_key.pem`.
  ...
  Returns:
      The compact user_authorization token written to the PaymentMandate.
  """
  ...
  authorization = get_mandate_signer().sign_mandate(
      cart_mandate=cart_mandate,
      payment_mandate_contents=payment_mandate.payment_mandate_contents,
  )
  payment_mandate.user_authorization = authorization.to_compact()
  tool_context.state["signed_payment_mandate"] = payment_mandate
  tool_context.state["signer_did"] = authorization.signer_did
...
def _generate_cart_mandate_hash(cart_mandate: CartMandate) -> str:
  """SHA-256 hex digest of the canonical CartMandate JSON.

  The full CartMandate (contents + merchant_authorization) is hashed so that
  the resulting digest binds the user's signature to the exact merchant-signed
  offer the user approved.
  """
  return MandateSigner.hash_object(cart_mandate)


def _generate_payment_mandate_hash(
    payment_mandate_contents: PaymentMandateContents,
) -> str:
  """SHA-256 hex digest of the canonical PaymentMandateContents JSON."""
  return MandateSigner.hash_object(payment_mandate_contents)
