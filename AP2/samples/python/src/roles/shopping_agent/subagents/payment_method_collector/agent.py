...
payment_method_collector = RetryingLlmAgent(
    ...
    3. Source payment methods in this order:
       a. Call `get_merchant_on_file_payment_methods` with the user's email.
          If it returns any aliases, label that group in the UI as
          "Saved with this merchant".
       b. Call `get_payment_methods` to fetch options from the Credentials
          Provider. Label those as "Other wallets / cards".
       Merge both lists into a single numbered picker. If merchant-on-file
       returns nothing, fall back to the CP list only.
    4. Ask the user to choose which of their forms of payment they would
       like to use for the payment. Remember that payment_method_alias AND
       which source it came from.
    5. Call the right token tool based on source:
       - If the chosen alias came from merchant-on-file, call
         `get_merchant_on_file_token`.
       - Otherwise call `get_payment_credential_token` (CP path).
    ...
    tools=[
        ...
        tools.get_merchant_on_file_payment_methods,
        tools.get_merchant_on_file_token,
    ],
)
